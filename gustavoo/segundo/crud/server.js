...
app.post('/compras', (req, res) => {
  const { description, distancia, tempo } = req.body;
  // if (!description, distancia) {
  //   res.status(400).json({ error: 'O nome do item é obrigatório' });
  //   return;
  // }
  db.run('INSERT INTO compras (description, distancia, tempo) VALUES (?, ?, ?)', [description, distancia, tempo], function (err) {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
    res.json({ id: this.lastID, description, distancia, tempo });
  });
});

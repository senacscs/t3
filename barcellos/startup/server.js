...
app.post('/compras', (req, res) => {
  ...
  const desc_formatado = description?.toLowerCase()
  const valor = valores[desc_formatado] ?? 0
  if (!description) {
    res.status(400).json({ error: 'O nome do item é obrigatório' });
    return;
  }
  db.run('INSERT INTO compras (description, valor) VALUES (?, ?)', [description, valor], function (err) {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
    res.json({ id: this.lastID, description, valor });
  });
});
